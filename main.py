...
class API_Harvest(App):
    ...
    def __init__(self, root):
        ...
        self.deaths = tk.Label(window, text='', font=("Comic Sans", 20), fg=("#ff0000"))
        self.deaths.grid(row=3)
        self.new_confirmed = tk.Label(window, text='', font=("Comic Sans", 20), fg=("#ff0000"))
        self.new_confirmed.grid(row=4)
        self.new_recovered = tk.Label(window, text='', font=("Comic Sans", 20), fg=("#ff0000"))
            ...
            for i in r["Countries"]:
                if i["Country"].lower() == lower_country:
                    ...
                    if total_new_confirmed > total_new_recovered:
                        self.country.configure(fg="#ff0000")
                        self.confirmed.configure(fg="#ff0000")
                        self.deaths.configure(fg="#ff0000")
                        self.new_confirmed.configure(fg="#ff0000")
                        self.new_recovered.configure(fg="#ff0000")
                    elif total_new_confirmed < total_new_recovered:
                        self.country.configure(fg="#0eff0a")
                        self.confirmed.configure(fg="#0eff0a")
                        self.deaths.configure(fg="#0eff0a")
                        self.new_confirmed.configure(fg="#0eff0a")
                        self.new_recovered.configure(fg="#0eff0a")
                    elif total_new_confirmed == total_new_recovered:
                        self.country.configure(fg="#ffff00")
                        self.confirmed.configure(fg="#ffff00")
                        self.deaths.configure(fg="#ffff00")
                        self.new_confirmed.configure(fg="#ffff00")
                        self.new_recovered.configure(fg="#ffff00")
        except:
